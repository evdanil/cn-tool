def browser_tips(filter_hint: str = "", search_hint: str = "", preview_focused: bool = False) -> str:
    ...
    if preview_focused:
        ...
        if search_hint:
            ...
            base = "Tips: Z=max, Ctrl+L=layout, Tab=back to list, Ctrl+Q=quit"
        else:
            base = "Tips: Ctrl+F/Alt+F=find, Z=max, Ctrl+L=layout, Tab=back to list, Ctrl+Q=quit"
    else:
        # Table is focused - Enter opens/navigates
        base = "Tips: Enter=open, Left/Alt+Up=up, Ctrl+F/Alt+F=find, Z=max, Ctrl+L=layout, Home/End=jump, Ctrl+Q=quit"
